...
import { colors, hexToAnsi, FRAME_BASE_COLOR, FRAME_RIPPLE_COLOR, RIPPLE_GLYPH, RIPPLE_BAND, RIPPLE_STEP_MS } from '../theme';
...
export class RecommendationsComponent implements TuiComponent {
    ...
    render(width: number, state: AppState): string[] {
        ...
        const { recommendations, selectedCategory, currentTrack, totalTracksInLibrary, debugMessage, terminalHeight } = state;
        ...
        if (recommendations.length === 0) {
            if (totalTracksInLibrary === 0) {
                ...
                lines.push(wrapInBorders(
                    `${colors.dim}Tip: export your liked songs as Liked_Songs.csv (Exportify), drop it in this project, and run \`npm run refresh:library\` or press r.${colors.reset}`, rowIndex++
                ));
            } else {
                lines.push(wrapInBorders(`${colors.dim}No harmonic matches found in library.${colors.reset}`, rowIndex++));
            }
            if (debugMessage) {
            ...
            for (const type of typesToRender) {
                ...
                if (tracks && tracks.length > 0) {
                    ...
                    if (selectedCategory === 'ALL') {
                        ...
                        lines.push(wrapInBorders(`${colors.bright}${typeColor}${type}${colors.reset}`, rowIndex++));
                    }
                    ...
                    tracks.forEach(track => {
                        ...
                        const normalizedInput = fullLineText.toUpperCase().padEnd(availableWidth, ' ').slice(0, availableWidth);
