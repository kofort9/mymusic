...
export class SpotifyProvider implements AudioFeatureProvider {
  ...
  async getAudioFeatures(trackId: TrackID, trackName?: string, artist?: string): Promise<AudioFeatures | null> {
    ...
    try {
      ...
      Logger.log(`[${this.getName()}] Fetched: BPM=${features.tempo}, Key=${features.key}, Mode=${features.mode}, TimeSig=${features.time_signature}`);
      ...
      Logger.error(`[${this.getName()}] Audio features fetch error for ${trackId}: ${errorDetails}`, error);
      
      // Log deprecation warning if we get a specific error
      if (err.statusCode === 410 || err.statusCode === 404) {
        Logger.error(`[${this.getName()}] WARNING: Endpoint may be deprecated! Consider switching to an alternative provider.`);
      }
