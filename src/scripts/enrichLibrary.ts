...
export async function enrichLibrary(options: EnrichmentOptions = {}): Promise<void> {
  ...
  if (!tracker.canMakeRequest()) {
    console.log(`❌ API limit reached! Resets on ${tracker.getResetDate().toLocaleDateString()}\n`);
    return;
  }
  ...
  if (options.interactive !== false) {
    ...
    console.log(
      `[1] Enrich all ${incompleteTracks.length} tracks (uses ${incompleteTracks.length} API calls)`
    );
